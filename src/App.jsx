...
function sendNotif(title, body){
  if("Notification" in window && Notification.permission === "granted"){
    try { new Notification(title, { body, icon:"/favicon.ico", tag:"tlc-chat" }); } catch(e){}
  }
}
...
function useChatSubscription({ myName, onNewMsg }){
  ...
  useEffect(() => {
    ...
    .then(rows => {
      ...
      const msgs = rows.map(dbToMsg);
      onNewMsg(msgs, "init");
      if(rows.length) lastIdRef.current = rows[rows.length-1].id;
    })
    .catch(() => {});

    // Poll every 4s for new messages from OTHER people
    const poll = setInterval(async () => {
      try {
        const r = await fetch(
          `${SUPABASE_URL}/rest/v1/tlc_chat?order=created_at.asc&id=gt.${lastIdRef.current}&limit=20`,
          { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
        );
        const rows = await r.json();
        if(!Array.isArray(rows) || rows.length === 0) return;
        lastIdRef.current = rows[rows.length-1].id;
        const newMsgs = rows.map(dbToMsg);
        onNewMsg(newMsgs, "poll");
        // Notify for messages from others when tab is not focused
        newMsgs.forEach(m => {
          if(m.name !== myNameRef.current.trim() && document.hidden){
            sendNotif(`TLC Chat — ${m.name}`, m.text);
          }
        });
      } catch(e){}
    }, 4000);
...
function ChatTab(){
  ...
  useChatSubscription({
    myName: name,
    onNewMsg: (incoming, type) => {
      if(type === "init"){
        setMsgs(incoming);
        setConnStatus("live");
      } else {
        // Poll: only add messages we don't already have by real ID
        setMsgs(prev => {
          const existingIds = new Set(prev.filter(m => typeof m.id === "number").map(m => m.id));
  ...
  const send = async () => {
    ...
    setSending(true);
    setText("");
    // Add optimistic message with a temp string ID
    const tempId = `opt_${Date.now()}`;
    const c = getChatColor(nm);
    const optimistic = { id:tempId, name:nm, initials:initials(nm), color:c.color, bg:c.bg, bc:c.bc, text:txt, time:new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:true}), me:true };
    setMsgs(m => [...m, optimistic]);
    try {
      const saved = await sendChatMsg(nm, txt);
      // Replace optimistic with confirmed real row (swap temp id → real id)
      // This prevents the poll from adding it as a duplicate
      if(saved && saved.id){
        setMsgs(m => m.map(msg => msg.id === tempId ? { ...dbToMsg(saved), me:true } : msg));
      }
    } catch(e){
      // Keep optimistic on failure — mark as unsent
      setMsgs(m => m.map(msg => msg.id === tempId ? { ...msg, failed:true } : msg));
    }
    setSending(false);
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 120px)", minHeight:500 }}>
      ...
      <div style={{ padding:"12px 18px", borderBottom:"1px solid rgba(255,255,255,.06)", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          {/* Notification button */}
          {notifPerm === "default" && (
            <button onClick={enableNotifs} style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 10px", background:"rgba(201,150,42,.1)", border:"1px solid rgba(201,150,42,.25)", borderRadius:100, cursor:"pointer", fontFamily:"'JetBrains Mono',monospace", fontSize:9, letterSpacing:"0.1em", textTransform:"uppercase", color:GOLD_LT }}>
              🔔 Enable Notifications
            </button>
          )}
          {notifPerm === "granted" && (
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:9, letterSpacing:"0.1em", textTransform:"uppercase", color:"#4caf82" }}>🔔 Notifs on</span>
          )}
          {/* Live dot */}
          ...
          return (
            <div key={m.id || i} className="chat-msg-enter" style={{ display:"flex", gap:9, alignItems:"flex-end", flexDirection: isMe ? "row-reverse" : "row" }}>
              ...
              <div style={{ maxWidth:"72%" }}>
                ...
                <div style={{ background: isMe ? `linear-gradient(135deg, rgba(201,150,42,.25), rgba(201,150,42,.1))` : "#1a1a20", border:`1px solid ${m.failed ? "rgba(224,82,82,.4)" : isMe ? "rgba(201,150,42,.3)" : "rgba(255,255,255,.07)"}`, borderRadius: isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px", padding:"9px 13px", boxShadow: isMe ? "0 2px 12px rgba(201,150,42,.15)" : "0 2px 8px rgba(0,0,0,.2)" }}>
                  <div style={{ fontSize:13, lineHeight:1.55, color:"#f0ede8" }}>{m.text}</div>
                  {m.failed && <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:8, color:"#e05252", marginTop:3, letterSpacing:"0.08em" }}>⚠ Failed to send</div>}
                </div>
